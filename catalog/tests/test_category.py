...
def test_categories_ordered_by_display_order_then_name():
    Category.objects.create(name="Desserts", display_order=2)
    Category.objects.create(name="Starters", display_order=1)
    Category.objects.create(name="Beverages", display_order=1)
...
def test_slug_autopopulated_from_name():
    category = Category.objects.create(name="Hot Drinks")
